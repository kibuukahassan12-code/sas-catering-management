    ...
    def decorated_function(*args, **kwargs):
        ...
        try:
            if hasattr(current_user, 'has_permission'):
                if current_user.has_permission("admin.manage"):
                    return f(*args, **kwargs)
        except Exception:
            pass
        
        # If all checks fail, show 403 but log the user info for debugging
        ...
        try:
            ...
            current_app.logger.error(
                ...
                f"ID: {user_id}, "
                f"Role: {role_name}, "
                f"is_super_admin: {current_user.is_super_admin() if hasattr(current_user, 'is_super_admin') else 'N/A'}"
            )
        except Exception as e:
            current_app.logger.error(f"Error logging access denial: {e}")
        
        return render_template("errors/403.html", permission="admin.manage"), 403
...
def test_access():
    ...
    user_info = {
        ...
        "has_admin_manage": current_user.has_permission("admin.manage") if hasattr(current_user, 'has_permission') else False,
