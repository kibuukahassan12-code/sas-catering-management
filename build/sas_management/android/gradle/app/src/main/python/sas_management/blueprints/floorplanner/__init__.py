...
def new_floorplan(event_id):
    ...
    try:
        ...
        if existing:
            flash("A floor plan already exists for this event. Opening editor...", "info")
        ...
        flash(f"Floor plan '{floorplan.name}' created! Start designing your layout.", "success")
        return redirect(url_for("floorplanner.editor", id=floorplan.id))
    except Exception as e:
        current_app.logger.exception(f"Error creating floor plan: {e}")
        flash(f"Error creating floor plan: {str(e)}", "danger")
...
def editor(id):
    ...
    try:
        ...
        # Parse layout JSON
        try:
            layout_data = json.loads(floorplan.layout_json) if floorplan.layout_json else {"objects": [], "meta": {}}
