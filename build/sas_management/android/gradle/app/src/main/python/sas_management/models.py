...
class User(UserMixin, db.Model):
    ...
    def has_permission(self, code):
        """Check if user has a specific permission. Supports wildcard matching (e.g., 'events.*')."""
        # SuperAdmin bypasses everything
        if self.is_super_admin():
            return True
        
        # Check permissions from all assigned roles (many-to-many)
        for role in self.roles:
            # Check exact permission match
            if any(p.code == code for p in role.permissions):
                return True
            
            # Check wildcard permissions
            if code and '.' in code:
                permission_group = code.split('.')[0]
                wildcard_permission = f"{permission_group}.*"
                if any(p.code == wildcard_permission for p in role.permissions):
                    return True
        
        # Fallback to legacy single role
        if self.role_obj:
            if any(p.code == code for p in self.role_obj.permissions):
                return True
            if code and '.' in code:
                permission_group = code.split('.')[0]
                wildcard_permission = f"{permission_group}.*"
                if any(p.code == wildcard_permission for p in self.role_obj.permissions):
                    return True
        
        return False
        
        # Check wildcard permissions (e.g., "events.*" matches "events.view", "events.edit", etc.)
        if code and '.' in code:
            permission_group = code.split('.')[0]
            # Check if user has wildcard permission for this group
            wildcard_permission = f"{permission_group}.*"
            if any(p.code == wildcard_permission for p in self.role_obj.permissions):
                return True
    ...
    def has_role(self, role_name):
        ...
        if self.is_super_admin() and role_name.lower() == "superadmin":
            return True
        return any(role.name.lower() == role_name.lower() for role in self.roles) or \
               (self.role_obj and self.role_obj.name.lower() == role_name.lower())
...
class InventoryItem(db.Model):
    ...
    # Relationships
    order_items = db.relationship("HireOrderItem", back_populates="inventory_item")
...
class HireOrder(db.Model):
    ...
    client = db.relationship("Client")
    event = db.relationship("Event")
    items = db.relationship("HireOrderItem", back_populates="order", cascade="all, delete-orphan")
    
    def __repr__(self):
        return f'<HireOrder {self.reference}>'
...
class HireOrderItem(db.Model):
    ...
    id = db.Column(db.Integer, primary_key=True)
    hire_order_id = db.Column(db.Integer, db.ForeignKey("hire_order.id"), nullable=False)
    inventory_item_id = db.Column(db.Integer, db.ForeignKey("inventory_item.id"), nullable=False)
    quantity_rented = db.Column(db.Integer, nullable=False, default=1)
    ...
    order = db.relationship("HireOrder", back_populates="items")
    inventory_item = db.relationship("InventoryItem", back_populates="order_items")
    
    def __repr__(self):
        return f'<HireOrderItem {self.id}>'
...
class FloorPlan(db.Model):
    ...
    layout_json = db.Column(db.Text, nullable=False, default='{"objects": [], "meta": {"zoom":1,"pan":{"x":0,"y":0},"grid":true}}')
