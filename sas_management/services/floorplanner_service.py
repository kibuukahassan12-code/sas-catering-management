...
def create_floorplan(event_id: int, user_id: int, name: str = None) -> FloorPlan:
    ...
    try:
        ...
        default_data = '{"objects": [], "meta": {"zoom": 1, "pan": {"x": 0, "y": 0}, "grid": true}}'
        floorplan = FloorPlan(
            event_id=event_id,
            name=name,
...
def update_floorplan(id: int, json_data: str) -> FloorPlan:
    ...
    try:
        ...
        floorplan.data = json_data  # Primary field - save here first
        floorplan.layout_json = json_data  # Keep for compatibility
...
def export_pdf(id: int) -> bytes:
    ...
    try:
        ...
        elements.append(Paragraph(f"<b>Event:</b> {event.title}", event_style))
        elements.append(Paragraph(f"<b>Date:</b> {event.date.strftime('%B %d, %Y') if event.date else 'N/A'}", event_style))
        if event.venue_obj:
            elements.append(Paragraph(f"<b>Venue:</b> {event.venue_obj.name}", event_style))
