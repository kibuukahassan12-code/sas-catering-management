"""
Safe Auto-Fix Script for service_events.title Column

This script safely adds the missing 'title' column to service_events table.
...
def fix_service_events_title_column(db_path=None):
    """
    Fix missing 'title' column in service_events table.
    ...
    try:
        ...
        if check_column_exists(cursor, "service_events", "title"):
            conn.close()
            return True, "[OK] service_events.title already exists"
        
        # Add title column - SQLite-safe
        try:
            cursor.execute("ALTER TABLE service_events ADD COLUMN title TEXT")
            conn.commit()
            conn.close()
            return True, "[FIX] service_events.title added"
        except sqlite3.OperationalError as e:
            error_msg = str(e).lower()
            if "duplicate column" in error_msg or "already exists" in error_msg:
                conn.close()
                return True, "[OK] service_events.title already exists (detected during add)"
            else:
                conn.rollback()
                conn.close()
...
if __name__ == "__main__":
    ...
    success, message = fix_service_events_title_column()
    print(message)
